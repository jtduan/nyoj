...
## other
    ...
    - 完全背包（恰好装满：去掉该物品时为-1，则不进行max比较）
    - 位运算，包括异或和 统计各位的个数（Leetcode137）
...
## 解题步骤
    ...
    - 使用双链表遍历,双指针同向滑动窗口（Leetcode424),双指针反向
    - 考虑使用和值sum[i]=a[0]+...+a[i]观察变化（nyoj109)
    - 考虑贪心算法
    - 考虑二分法和枚举法
    - 树
    - 图
        ...
        - Dijkstra算法,每次选出一条最短路加入集合
        - A*算法
        - Prim算法，与dijkstra非常相似，每次选出最短路加入集合，不包括起点和终点已经在集合中。
        - SFPA算法
        - 网络流算法，每次“按一定规律”选出一条最短路径（Dijkstra/SFPA）,更新最大流
        - 二分图最大匹配算法，每次“按一定规律”找出更长的路径（DFS/BFS),更新路径
    - other
        - huffman
        - 数学算法，计算几何，点与直线的关系，点到直线的距离，叉积的意义(两条直线围成的平行四边形面积)
        - 排序算法
        - 二分查找
        - 并查集,加权并查集(`nyoj207`),以及并查集扩展（敌人的敌人是朋友）


   经典题目：
   LeetCode424 双指针（动态规划）
   NYOJ37 回文字符串（动态规划）